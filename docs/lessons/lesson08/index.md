...
- 色覚を説明する 2 つの古典理論（三色説と反対色説）の主張を整理する
- 現代的には 2 つの理論が**段階説（統合説）**として統合されていることを理解する
- 3 色型色覚（C 型）のしくみを S・M・L 錐体の刺激比率で説明できる
- 色覚タイプ（P 型・D 型・T 型・A 型）と対応する錐体を覚える
- 色覚を「異常」ではなく「**多様性**」として捉える視点を持つ
...
[lesson07](/lessons/lesson07/)で、網膜には 3 種類の錐体（S・M・L）があることを学びました。一般的なヒトの色覚は、これら 3 種類の錐体すべてが機能している**3 色型色覚（トリクロマシー、trichromacy）**です。**C 型色覚（Conventional type）**とも呼ばれます。
...
::: warning 用語について
「色覚異常」は医学用語・検定試験で使われる表現です。「色盲」「色弱」は歴史的経緯の説明を除き使いません。当事者にとって「異常」という言葉は強い印象を与えるため、社会的な文脈では「色覚特性」「色覚の多様性」が推奨されます。
:::
...
色覚特性の出現頻度は地域・人種によって異なりますが、日本人での代表的な値は次のとおりです。

| 対象 | 全体の頻度 | 内訳 |
|------|----------|------|
| 男性 | **約 5%（20 人に 1 人）** | P 型 約 1.5%・D 型 約 3.5%・T 型 非常にまれ・A 型 非常にまれ |
| 女性 | **約 0.2%（500 人に 1 人）** | — |

::: info 男女差の理由
P 型・D 型は X 染色体上の遺伝子の変化によって生じます。男性は X 染色体を 1 本しか持たないため発現しやすく、女性は 2 本のうち 1 本が正常なら C 型として発現します。詳しくは [lesson13](/lessons/lesson13/) で扱います。
:::

### D 型が最多であること

日本人男性では **D 型が約 3.5%・P 型が約 1.5%** で、**D 型のほうが約 2 倍多い**点は試験で問われやすいポイントです。
...
::: tip このレッスンは「概論」
色覚の多様性は本サイトの第 4 章（[lesson12](/lessons/lesson12/) 〜 [lesson16](/lessons/lesson16/)）で詳しく扱います。本レッスンでは「色覚説」とセットで概論として押さえてください。
:::
...
| C 型色覚 | Conventional type の略。3 色型色覚の一般的な状態 |
