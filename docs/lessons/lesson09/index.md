...
3. **アクセシビリティ基準も明度ベース**: Webアクセシビリティ基準（WCAG AA基準）のコントラスト比も明度に基づいています
...
::: info マンセル表色系
代表的な色立体の1つが**マンセル表色系**です。色相（H）・明度（V）・彩度（C）を H V/C という記号で表します（例：5R 4/14 = 色相5R、明度4、彩度14）。
:::
...
三属性の組み合わせ方によって、色はまず有彩色と無彩色に分かれます。さらに有彩色は、純色・清色・濁色へと分類されます。色の分類は[lesson09](/lessons/lesson09/)で学びます。
...
- テキストと背景の明度差が大きいほど視認性・可読性が高い（WCAG AA基準のコントラスト比も明度ベース）
