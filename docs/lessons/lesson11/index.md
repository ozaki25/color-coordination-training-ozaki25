...
**PCCS（Practical Color Co-ordinate System）**は、**日本色彩研究所**が発表した色体系です。「実用的な色彩座標システム」という名の通り、**配色教育・デザイン実務での使いやすさ**を最優先に設計されています。
...
記号の大文字は基本の色みを表します（R＝赤、O＝橙、B＝青 など）。先頭の小文字は「どちら寄りか」を示す補助記号です。たとえばpRはp（purple＝紫）寄りのR（赤）、rOはr（red＝赤）寄りのO（橙）、gBはg（green＝緑）寄りのB（青）という意味です。試験では大まかな色みがつかめれば十分です。
:::
...
PCCSのトーンマップでは、横軸が彩度（右ほど鮮やか）、縦軸が明度（上ほど明るい）に対応します。vividは右中央、paleは左上、darkは左下に位置します。この位置関係を頭に入れると、トーン同士の関係が理解しやすくなります。
:::

![PCCSトーンマップ。縦軸=明度・横軸=彩度の上に、vivid・bright・pale・soft・dull・darkなど12のトーンが配置された図。赤系で例示。](/diagrams/pccs-tone-map.svg)
...
例：同じ青系でbright（明るい）とdark（暗い）を組み合わせる → 青の濃淡によるグラデーション感
...
::: info 「補色」と「混同色」を混同しない
**補色は色相環の位置で決まる**（色の理論上の対比）、**混同色は錐体の応答で決まる**（生理的な見え方）、と決まり方が違います。赤と緑は色相環では補色の位置にありますが、P型・D型に見分けにくいのは「補色だから」ではなく「L錐体／M錐体の応答が似てしまうから」です。詳しくは [lesson16](/lessons/lesson16/) で扱います。
:::
...
- **トーンの名称と特徴**を覚える：特にvivid・bright・pale・deep・dull・darkは頻出
