...
補色は色相環で正反対の関係にある色どうしを指します。一方、色覚特性で見分けにくい色は混同色線という別の考え方で説明されます。赤と緑は色相環では離れた関係ですが、P型（1型）・D型（2型）に見分けにくいのは補色だからではありません。詳しくは [lesson16](/lessons/lesson16/) で学びます。
:::
