...
- P型とD型の見え方の違いと共通点を理解する

## P型色覚（1型色覚）とは

P型色覚（1型色覚）は、**L錐体（長波長錐体）**の欠損または機能変化によって生じる色覚特性です。「P」は英語の **Protan（プロタン）**に由来します。Protanは「最初の（プロト＝first）」という意味で、歴史的に最初に詳しく研究された色覚特性であることからこの名前が付いています。

L錐体は**長波長の光（赤〜黄色系）**に感受します。L錐体が正常に機能しないと、長波長の光に対する感度が低下し、**赤みを感じにくくなります**。

::: info P型の強度と弱度
- **P型強度（Protanopia）**: L錐体が完全に欠損している状態。より強く色の混同が生じる
- **P型弱度（Protanomaly）**: L錐体の感度が変化しているが存在する状態。強度より影響は小さい
...
P型に特有の代表例として、**信号機の赤が暗く見える**ことが挙げられます。現在の信号機は**点灯位置（上が赤・下が青）**で判断できるよう設計されており、これはUDの考え方が反映された例です。

このほか、赤と緑の路線が同時に使われる路線図、赤と緑の系列を使ったグラフ、赤い果実と緑の葉が混ざる場面（熟れ具合の判断）、赤と緑のケーブルの判別など、さまざまな場面で見分けにくさが生じます。日常事例の網羅的な整理は [lesson20](/lessons/lesson20/) で扱います。
...
実用的な色のUDでは、P型とD型を個別に区別するよりも、**「赤緑の混同が生じる可能性がある」**と考えて両方に対応した配色を心がけることが重要です。
