# lesson25: シミュレーションツール — 色覚特性の見え方を確認する
...
C型（一般的な色覚）の人が色のUDを実践するうえで、最大の障壁の一つは「P型やD型の人の見え方が想像しにくい」ことです。

「この赤と緑は十分区別できる」と思っていても、P型・D型の人には同じ色に見えているかもしれません。自分の見え方が「標準」だという思い込みは、誰にでも自然に生まれます。

**色覚シミュレーションツールを使うと、C型の人が「P型にはこう見える」を自分の目で確認**できます。これにより、問題のある配色を事前に発見して改善することができます。
...
最も広く知られたシミュレーションアプリの一つです。スマートフォンのカメラを通して、リアルタイムでP型・D型・T型の見え方をシミュレーションできます。

- カメラ越しに目の前のものをそのまま確認できる
- P型（1型2色覚）・D型（2型2色覚）・T型（3型2色覚）に対応
...
| 対象 | 最低基準（AA） | 推奨基準（AAA） |
|------|--------------|----------------|
| 標準テキスト（18pt未満） | **4.5:1** 以上 | 7:1 以上 |
| 大きなテキスト（18pt以上、または14pt以上の太字） | **3:1** 以上 | 4.5:1 以上 |
| UIコンポーネント・グラフィック要素 | **3:1** 以上 | — |
...
シミュレーションを行う際は、以下のチェックリストを参考にしてください。
