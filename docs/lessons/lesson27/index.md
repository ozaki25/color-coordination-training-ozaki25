# lesson27: チェックの方法 — シミュレーションとコントラスト確認
...
**2. 色のシミュレータ（iOS / Android）**

写真をアプリに読み込んでシミュレーションを適用できる日本語向けのアプリです。過去に作成した資料の画像ファイルを読み込んで確認するのに便利です。
...
試験では「**標準テキストは4.5:1以上**」「**大きなテキストや図形要素は3:1以上**」が目安として問われることがあります。この2つの数値は確実に覚えましょう。
:::
...
**可能であれば、色覚特性のある人や高齢者に実際に確認してもらうことが最も確実**です。設計プロセス全体の流れは [lesson26](/lessons/lesson26/) も参照してください。
...
- シミュレーションが必要な理由：「**C型の人がP型・D型の見え方を体験できる**」から
- シミュレーターの限界：**近似**であり個人差がある。**最終確認は当事者へのヒアリング**
- コントラスト比の基準（WCAG AA）：**標準テキスト 4.5:1以上、大きな文字・図形 3:1以上**
- 確認すべき項目：シミュレーション、**グレースケール変換**、コントラスト比、**色以外の手がかり**
- Photoshop・Illustratorには**校正設定**としてP型・D型シミュレーション機能が内蔵
